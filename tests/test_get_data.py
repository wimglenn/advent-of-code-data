...
def test_get_from_server_has_error(pook: pook_mod) -> None:
    url = "https://adventofcode.com/2018/day/1/input"
    pook.get(url, reply=418)
    with pytest.raises(AocdError(f"HTTP 418 at {url}")): # type: ignore[call-overload] # using pytest-raisin
        aocd.get_data(year=2018, day=1, session="bogus")


def test_get_data_uses_current_date_if_unspecified(pook: pook_mod, freezer: FrozenDateTimeFactory) -> None:
    mock = pook.get(
...
def test_saved_data_is_reused_if_available(aocd_data_dir: Path, pook: pook_mod) -> None:
    mock = pook.get(
        url="https://adventofcode.com/2018/day/1/input",
        response_body="fake data for year 2018 day 1",
    )
...
def test_server_error(pook: pook_mod, caplog: pytest.LogCaptureFixture) -> None:
...
def test_puzzle_not_available_yet(pook: pook_mod, caplog: pytest.LogCaptureFixture) -> None:
    mock = pook.get(
        url="https://adventofcode.com/2101/day/1/input",
        response_body="Not Found",
        reply=404,
    )
    with pytest.raises(PuzzleLockedError("2101/01 not available yet")): # type: ignore[call-overload] # using pytest-raisin
        aocd.get_data(year=2101, day=1)
    assert mock.calls == 1


def test_puzzle_not_available_yet_block(pook: pook_mod, caplog: pytest.LogCaptureFixture, mocker: MockerFixture) -> None:
    mock = pook.get(
        url="https://adventofcode.com/2101/day/1/input",
        response_body="Not Found",
        reply=404,
...
def test_req_headers(pook: pook_mod) -> None:
...
def test_data_is_cached_from_successful_request(aocd_data_dir: Path, pook: pook_mod) -> None:
    pook.get(
        url="https://adventofcode.com/2018/day/1/input",
        response_body="fake data for year 2018 day 1",
    )
...
def test_corrupted_cache(aocd_data_dir: Path) -> None:
    cached = aocd_data_dir / "testauth.testuser.000" / "2018_01_input.txt"
...
def test_race_on_download_data(mocker: MockerFixture, pook: pook_mod) -> None:
    pook.get(
        url="https://adventofcode.com/2018/day/1/input",
        response_body="fake data for year 2018 day 1",
        times=2,
    ...
    def logging_os_open(path, *args, **kwargs): # type: ignore[no-untyped-def] # no clue how to type hint this
    ...
    def generate_open(real_open): # type: ignore[no-untyped-def] # no clue how to type hint this
        def open_impl(file, *args, **kwargs): # type: ignore[no-untyped-def] # no clue how to type hint this
            ...
            def write(*args, **kwargs): # type: ignore[no-untyped-def] # no clue how to type hint this
                write_evt.wait()
    ...
    mocker.patch("io.open", side_effect=generate_open(io.open)) # type: ignore[no-untyped-call] # no clue how to type hint this
